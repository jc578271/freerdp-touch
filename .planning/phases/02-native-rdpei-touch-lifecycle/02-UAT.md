...
- test: 2
  idea: "Long-press (press-and-hold) no longer shows the Windows right-click context box. Long-press is a Phase 03 gesture (one of the three selected: drag, long-press, pinch) — not yet implemented. User reports 'nữa' (used to work) -> possible regression OR Windows press-and-hold suppressed by touch jitter (Test 1 log shows held contacts wobble 1-3px, may exceed Windows press-and-hold movement tolerance). Needs targeted long-press test in a fresh session to capture held-contact log and confirm whether contacts deliver cleanly stationary."
  deferred_at: 2026-08-06
```